+++
title = 'golangci-lint使用'
date = '2025-05-20T13:13:04+08:00'
author = 'jianlu'
draft = false
description = "golang静态代码检查工具"
aliases = ["golang", "static check", "golangci-lint"]
+++
...
```shell
# 1. 在项目根目录(go.mod)新建.golangci.toml文件 .golangci.yaml .golangci.json都可以
touch .golangci.toml
# 2. 在.golangci.toml文件中添加需要检查的规则 
```
...
paths = [".*\\.my\\.go$", "lib/bad.go"]
paths-except = [".*\\.my\\.go$", "lib/bad.go"]
...
[output.formats.checkstyle]
path = "./path/to/output.xml"

[output.formats.code-climate]
path = "./path/to/output.json"

[output.formats.junit-xml]
path = "./path/to/output.xml"
extended = true

[output.formats.teamcity]
path = "./path/to/output.txt"
